...
const NewTrainigPage: React.FC = () => {
    // the location is to get the staion ID value from the Student Login Page
    const location = useLocation();
    const locState = location.state as { station?: any; } | null;
    const station = locState?.station;

    // extract the studyunit id to use it later to fetch data 
    const studyUnitId = station?.studyUnitId;

    // states we need in the code
    ...
    // answers saved here in the front => index is a number and taksid as key string (this what i need to send to backend when submited)
    const [userAnswers, setUserAnswers] = useState<{ [key: string]: number }>({});

    // collect the data i need from DB => StudyUnit
    useEffect(() => {
        if (!studyUnitId) {
            setLoading(false);
            return;
    ...
    const stationIdValue = station?.id;

    // stable shuffled order of option indices per question (so correct answer isn't always first)
    const optionsOrderByTaskId = useMemo(() => {
        const map: Record<string, number[]> = {};
        questions.forEach((q) => {
            if (q.type === "SelectionQuestion") {
                const opts = (q as SelectionQuestion).options;
                map[q.id] = shuffleIndices(opts?.length ?? 0);
            }
        });
        return map;
    }, [thisStudyUnit?.id]);

    // function uniqe to go to spacific tasks (when the user click on task not on answer), need to use jumpToTask here (backend)
    ...
    // handle the student choise (need to use submitAnswers for backend)
    const handleAnswerSelect = (taskId: string, chosenIndex: number) => {
        ...
        /// dealy only for answer pick visuals (turn green)
        setTimeout(() => {
    ...
    // only calculations for the exam end  part
                        ...
                        {currentQuestion && currentQuestion.type === "SelectionQuestion" && (
                            <div className="training-content-body">
                                ...
                                <div className="answer-content">
                                    {((optionsOrderByTaskId[currentQuestion.id] ?? (currentQuestion as SelectionQuestion).options.map((_, i) => i)).map((originalIndex) => {
                                        const option = (currentQuestion as SelectionQuestion).options[originalIndex];
                                        const isChecked = userAnswers[currentQuestion.id] === originalIndex;
                                        return (
                                            <div key={originalIndex} className="student-answer-list">
                                                <div className={isChecked ? "check-container-checked" : "check-container-unchecked"}>
                                                    ...
                                                    <span className="checkbox-text" onClick={() => handleAnswerSelect(currentQuestion.id, originalIndex)}>בחר תשובה</span>
                                                </div>
